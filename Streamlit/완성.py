...
with tab_admin:
    ...
    with adm_t1:
        ...
        with st.form("admin_reg_form"):
            ...
            with c1:
                st.markdown("### 기본 정보")
                sup_n = st.text_input("공급처명")
                itm_n = st.text_input("품목명")
                cat_n = st.text_input("카테고리")
            with c2:
                st.markdown("### 발주 설정")
                u_url = st.text_input("주문 URL (선택 사항)")
                u_unit = st.text_input("주문 단위 (예: 박스)")
                moq = st.number_input("MOQ", min_value=1)
                u_prc = st.number_input("단가", min_value=0)
            
            st.divider() # 사장님 요청 구분선
            st.markdown("### 재고 및 단위 환산 설정")
            cc1, cc2, cc3 = st.columns(3)
            # 요청하신 순서: 재고관리단위 -> 환산계수 -> 안전재고
            b_unit = cc1.text_input("재고 관리 단위")
            c_fact = cc2.number_input("환산 계수", min_value=1)
            s_stck = cc3.number_input("안전재고", min_value=0)
            
            if st.form_submit_button("전체 데이터 등록 실행", type="primary"):
                # URL 제외 필수 입력 검증
                if not all([sup_n, itm_n, cat_n, u_unit, b_unit]):
                    st.error("🚨 URL을 제외한 모든 필수 항목을 채워주세요.")
                else:
                    # 중복 체크 후 순차 등록 로직 (SUPPLIERS -> ITEMS -> DETAILS -> STOCKS)
